...
describe('Create Note API', () => {
  ...
  it('creates a note and subsequent GET returns it', async () => {
    ...
    const body = await getRes.json() as { data: { attributes: { notes: Array<{ content: string; author: { id: string; name: string } }> } } };
  ...
  it('created note has server-generated id', async () => {
    ...
    const body = await getRes.json() as { data: { attributes: { notes: Array<{ id: string }> } } };
  ...
  it('created note has server-generated created_at', async () => {
    ...
    const body = await getRes.json() as { data: { attributes: { notes: Array<{ created_at: string }> } } };
    const note = body.data.attributes.notes[body.data.attributes.notes.length - 1];
  ...
  it('returns 400 when data.attributes.content is missing', async () => {
    ...
    const body = await res.json() as { errors: Array<{ code: string; source: { pointer: string } }> };
